class Player {
  constructor(scene) {
    this.scene = scene
  }

  initialize(x, y) {
    this.play('right')

    this.moveHistory = []

    this.x = x
    this.y = y
    this.gameObject.x = this.x * 16
    this.gameObject.y = this.y * 16

    this.gameObject.setDepth(10)
  }

  preload() {
    const { scene } = this
    scene.load.spritesheet(
      'player', 
      './assets/player_spritesheet.png',
      { frameWidth: 16, frameHeight: 16 }
    );
  }

  create(x, y) {
    const { scene } = this
    const gameObject = scene.add.sprite(
      0,
      0,
      'player'
    );

    if (!scene.anims.get('idle')) {
      scene.anims.create({
        key: 'idle',
        frames: scene.anims.generateFrameNumbers('player', { start: 0, end: 1 }),
        frameRate: 2,
        repeat: -1
      });
    }

    if (!scene.anims.get('right')) {
      scene.anims.create({
        key: 'right',
        frames: scene.anims.generateFrameNumbers('player', { start: 8, end: 15 }),
        frameRate: 8,
        repeat: -1
      });
    }

    if (!scene.anims.get('left')) {
      scene.anims.create({
        key: 'left',
        frames: scene.anims.generateFrameNumbers('player', { start: 16, end: 23 }),
        frameRate: 8,
        repeat: -1
      });
    }

    gameObject.setOrigin(0)
    this.gameObject = gameObject
    this.emitter = new Phaser.Events.EventEmitter();

    this.initialize(x = 0, y = 0)

    this.deltaX = 0
    this.deltaY = 0
  }

  canMove(map, deltaX, deltaY) {
    const gameObject = this.gameObject

    if (deltaX || deltaY) {
      const newX = this.x + deltaX
      const newY = this.y + deltaY

      if (deltaX > 0) {
        this.play('right')
      } else if (deltaX < 0) {
        this.play('left')
      }

      // check for walls
      const levelTile = map.getTileAt(newX, newY, false, 'level')
      const trailTile = map.getTileAt(newX, newY, false, 'trails')
      const itemTile = map.getTileAt(newX, newY, false, 'items')

      if (_.get(itemTile , 'properties.exit')) {
        return this.scene.canExit()
      }

      return (!levelTile && !trailTile)
    }
  }

  update() {
    const { scene } = this

    if (keyLEFTisDown) {
      this.deltaX = -1
    } else if (keyRIGHTisDown) {
      this.deltaX = 1
    }
    if (keyUPisDown) {
      this.deltaY = 1
    } else if (keyDOWNisDown) {
      this.deltaY = -1
    }

    const { map } = scene

    const gameObject = this.gameObject
    const moveX = (this.deltaX && this.canMove(map, this.deltaX, 0) && (keyLEFTisDown || keyRIGHTisDown)) ? this.deltaX : 0
    // only moveY if not moving X
    const moveY = (!this.deltaX && this.deltaY && this.canMove(map, 0, this.deltaY) && (keyUPisDown || keyDOWNisDown)) ? this.deltaY : 0

    if (moveX || moveY) {
      this.x = this.x + moveX
      this.y = this.y + moveY

      const prevMove = _.last(this.moveHistory)
      this.moveHistory.push([ moveX, moveY ])

      this.emitter.emit('move', this.x, this.y, moveX, moveY, prevMove)

    }

    if (this.deltaX || this.deltaY) {
      const newX = this.x * 16
      const newY = this.y * 16
      scene.tweens.add({
        targets: gameObject,
        x: newX,
        y: newY,
        duration: 100,
      });
    }
  }

  play(key) {
    if (this.activeAnim !== key) {
      const gameObject = this.gameObject
      this.activeAnim = key
      gameObject.anims.play(key, true)
    }
  }
}
